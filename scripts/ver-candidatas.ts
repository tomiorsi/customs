/**
 * Uso: npx tsx --require ./scripts/register-server-only-stub.cjs scripts/ver-candidatas.ts <partida> <texto> [max]
 */
import {
  descripcionPartida,
...
if (!p4 || !texto) {
  console.error("Uso: ver-candidatas.ts <partida> <texto> [max]");
  process.exit(1);
}
